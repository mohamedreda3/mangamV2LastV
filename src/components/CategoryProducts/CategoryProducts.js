...
const CategoryProducts = () => {
  ...
  const { categoryproduct, name, name_ar } = location.state;
  // console.log(name)
  // console.log(name_ar)
    ...
    axios
      ...
      .then((res) => {
        ...
        if (Array.isArray(res.data.message)) {
          ...
          let alloffers = [...res.data.message];
          let pushedoffers = alloffers.filter(
            (item) => item.products[0].category_id == categoryproduct
          );
  ...
  return (
    <div className="categoryproducts">
      <PagesHeader
        title={language == "ar" ? name_ar || name : name || name_ar}
      />
      {pageloading ? (
        <div>
          <Skeleton count={10} />
